...
export default async (req:Request,context:Context)=>{
  ...
  if(path==="proofs"&&req.method==="POST"){const b=await req.json();const p={id:uid(),client_name:b.client_name||"",image_before_url:b.image_before_url||"",image_after_url:b.image_after_url||"",category:b.category||"WeightLoss",testimonial_text:b.testimonial_text||"",created_at:now()};s.proofs.unshift(p);await save(user.id,s);return json(p,201)}
  ...
  if(path==="ai/score"&&req.method==="POST"){const b=await req.json();const l=s.leads.find(x=>x.id===b.lead_id);if(!l)return json({error:"Lead not found"},404);l.ai_score=score(l,s);l.updated_at=now();await save(user.id,s);return json({score:l.ai_score,label:l.ai_score>80?"Hot":l.ai_score>50?"Warm":"Cold"})}
  ...
  if(path==="interactions"&&req.method==="POST"){const b=await req.json();const l=s.leads.find(x=>x.id===b.lead_id);if(!l)return json({error:"Lead not found"},404);const msg=String(b.content||"");const sentiment=/love|excited|ready|yes|interested|when/i.test(msg)?"positive":/no|not interested|busy|expensive/i.test(msg)?"negative":"neutral";const reply=replyFor(l,msg);const i:Interaction={id:uid(),lead_id:l.id,type:b.type||"WhatsApp",content:msg,sentiment,ai_suggested_reply:reply,created_at:now()};s.interactions.unshift(i);l.ai_score=Math.max(l.ai_score,sentiment==="positive"?90:sentiment==="negative"?35:65);l.next_follow_up_at=new Date(Date.now()+172800000).toISOString();l.updated_at=now();await save(user.id,s);return json({reply,sentiment,ai_score:l.ai_score,next_follow_up_at:l.next_follow_up_at})}
  if(path==="pipeline"&&req.method==="PATCH"){const b=await req.json();const allowed=["New","Contacted","Assessment_Scheduled","Trial_Active","Converted","Lost"];if(!allowed.includes(b.status))return json({error:"invalid status"},400);const l=s.leads.find(x=>x.id===b.lead_id);if(!l)return json({error:"Lead not found"},404);l.status=b.status;l.updated_at=now();await save(user.id,s);return json(l)}
